...
const defaultData = {
    ...
    articles: [
        { id: 1, code: "ART-001", designation: "Ordinateur portable HP ProBook", prix: 450000, tva: 18 },
        { id: 2, code: "ART-002", designation: "Écran 24 pouces Dell", prix: 175000, tva: 18 },
        { id: 3, code: "ART-003", designation: "Souris sans fil Logitech", prix: 15000, tva: 18 },
        { id: 4, code: "ART-004", designation: "Clavier mécanique", prix: 35000, tva: 18 },
        { id: 5, code: "ART-005", designation: "Disque SSD 1To", prix: 85000, tva: 18 },
        { id: 6, code: "ART-006", designation: "Licence Windows 11 Pro", prix: 150000, tva: 18 },
        { id: 7, code: "ART-007", designation: "Imprimante laser HP", prix: 250000, tva: 18 },
        { id: 8, code: "ART-008", designation: "Onduleur APC 1000VA", prix: 125000, tva: 18 },
        { id: 9, code: "ART-009", designation: "Switch 8 ports Gigabit", prix: 65000, tva: 18 },
        { id: 10, code: "ART-010", designation: "RAM 16Go DDR4", prix: 75000, tva: 18 }
    ],
    proformas: [
        {
            id: 1,
            ...
            date: "2025-02-16",
            articles: [
                { id: 1, code: "ART-001", designation: "Ordinateur portable HP ProBook", prix: 450000, quantite: 2 },
                { id: 3, code: "ART-003", designation: "Souris sans fil Logitech", prix: 15000, quantite: 5 }
            ],
            montantHT: 975000,
            montantTVA: 175500,
            montantTTC: 1150500,
    ...
    ravitaillements: [
        ...
        },
        {
            ...
            articles: [
                ...
                { id: 8, nom: "Onduleur APC", quantite: 6 }
            ]
        },
        {
            id: 3,
            ...
            dateCreation: "2025-02-16",
            articles: [
                { id: 6, nom: "Licence Windows 11 Pro", quantite: 10 },
                { id: 9, nom: "Switch 8 ports", quantite: 7 },
                { id: 1, nom: "Ordinateur HP ProBook", quantite: 4 }
            ]
        }
    ]
};
...
function init() {
    ...
    if (!localStorage.getItem("pf_ravitaillements")) {
        localStorage.setItem("pf_ravitaillements", JSON.stringify(defaultData.ravitaillements));
    }
...
function genererCodeProforma() {
    ...
    return "PF-" + year + "-" + (proformas.length + 1).toString().padStart(4, "0");
}

function genererCodeRavitaillement() {
    const ravitaillements = getRavitaillements();
    const year = new Date().getFullYear();
    return "RAV-" + year + "-" + (ravitaillements.length + 1).toString().padStart(3, "0");
}
...
function loadClients() {
    ...
    dashboardClients.innerHTML = clients.slice(-5).reverse().map(client => {
        const clientRavitaillements = ravitaillements.filter(r => r.clientId === client.id);
        return `
                ...
                `;
    }).join("") || '<p style="text-align: center; padding: 20px;">Aucun client</p>';

    // Grid clients avec leurs lignes de ravitaillement
    ...
    clientsGrid.innerHTML = clients.map(client => {
        ...
        if (clientRavitaillements.length > 0) {
            ravitaillementsHtml = clientRavitaillements.map(rav => `
                        <div class="ravitaillement-card">
                            <div class="ravitaillement-header">
                                <div class="ravitaillement-title">
                                    <i class="fas fa-truck"></i>
                                    <span>Ligne de ravitaillement</span>
                                </div>
                                <span class="ravitaillement-code">${rav.code}</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                                <span class="ravitaillement-jour"><i class="fas fa-calendar"></i> ${rav.jour}</span>
                                <span style="color: #64748b; font-size: 12px;">Créé le ${rav.dateCreation}</span>
                            </div>
                            <div class="articles-ravitaillement">
                                ${rav.articles.map(a => `
                                    <div class="article-ravitaillement-item">
                                        <i class="fas fa-box"></i>
                                        <span>${a.nom}</span>
                                        <span class="qte">x${a.quantite}</span>
                                    </div>
                                `).join('')}
                            </div>
                            <div style="margin-top: 16px; display: flex; gap: 8px;">
                                <button class="btn-secondary" style="flex: 1; padding: 8px;" onclick="utiliserRavitaillement(${rav.id})">
                                    <i class="fas fa-shopping-cart"></i> Utiliser
                                </button>
                                <button class="btn-secondary" style="flex: 1; padding: 8px;" onclick="supprimerRavitaillement(${rav.id})">
        ...
        return `
                    <div class="client-card">
                        ...
                        <div style="display: flex; gap: 12px;">
                            ...
                            <button class="btn-ravitaillement" style="flex: 1;" onclick="openOffcanvas('ravitaillement', ${client.id})">
...
function loadRavitaillements() {
    ...
    grid.innerHTML = ravitaillements.map(rav => {
        const client = clients.find(c => c.id === rav.clientId);
        return `
                    <div class="client-card">
                        <div class="ravitaillement-card" style="margin: 0;">
                            <div class="ravitaillement-header">
                                <div class="ravitaillement-title">
                                    <i class="fas fa-truck"></i>
                                    <span>${rav.code}</span>
                                </div>
                                <span class="ravitaillement-code">${client?.nom || "Client inconnu"}</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; margin-bottom: 16px;">
                                <span class="ravitaillement-jour"><i class="fas fa-calendar"></i> ${rav.jour}</span>
                                <span style="color: #64748b;">Créé le ${rav.dateCreation}</span>
                            </div>
                            <div class="articles-ravitaillement" style="grid-template-columns: repeat(2, 1fr); margin-bottom: 16px;">
                                ${rav.articles.map(a => `
                                    <div class="article-ravitaillement-item">
                                        <i class="fas fa-box"></i>
                                        <span>${a.nom}</span>
                            ...
                            <div style="display: flex; gap: 8px;">
                                <button class="btn-primary" style="flex: 1; padding: 12px;" onclick="utiliserRavitaillement(${rav.id})">
                                    <i class="fas fa-shopping-cart"></i> Créer une commande
                                </button>
                                <button class="btn-secondary" style="flex: 1; padding: 12px;" onclick="supprimerRavitaillement(${rav.id})">
                                    <i class="fas fa-trash"></i> Supprimer
                                </button>
                            </div>
                        </div>
                ...
                `;
    }).join("") || '<p style="text-align: center; padding: 40px;">Aucune ligne de ravitaillement</p>';
}
...
function loadProformas() {
    ...
    dashboardProformas.innerHTML = proformas.slice(-5).reverse().map(proforma => {
        ...
        return `
                ...
                `;
    }).join("") || '<p style="text-align: center; padding: 20px;">Aucune proforma</p>';

    // Grid proformas avec liens de partage
    const proformasGrid = document.getElementById("proformas-grid");
    proformasGrid.innerHTML = proformas.map(proforma => {
        const client = clients.find(c => c.id === proforma.clientId);
...
function updateStats() {
    ...
    const proformas = getProformas();
    const ravitaillements = getRavitaillements();
    const totalTTC = proformas.reduce((sum, p) => sum + (p.montantTTC || 0), 0);
    ...
    document.getElementById("stats-ttc").textContent = formatCFA(totalTTC);
}
...
function getRavitaillementForm(clientId = null) {
    ...
    return `
                <form id="ravitaillementForm" onsubmit="enregistrerRavitaillement(event)">
                    ...
                    <div class="form-group">
                        <label>Jour de ravitaillement</label>
                        <select id="ravitaillement-jour">
...
function getCommandeForm(clientId = null) {
    ...
    return `
                <form id="commandeForm" onsubmit="enregistrerCommande(event)">
                    <div class="form-group">
                        <label>Client *</label>
                        <select id="commande-client" required onchange="chargerRavitaillementClient(this.value)">
                            <option value="">Sélectionner un client</option>
                            ${clients.map(c => `<option value="${c.id}" ${clientId == c.id ? "selected" : ""}>${c.nom}</option>`).join('')}
                        </select>
                    </div>
...
function chargerRavitaillementClient(clientId) {
    if (!clientId) return;

    const ravitaillements = getRavitaillements();
    const clientRavitaillements = ravitaillements.filter(r => r.clientId == clientId);

    if (clientRavitaillements.length > 0) {
        // Utiliser le dernier ravitaillement
        const dernierRav = clientRavitaillements[clientRavitaillements.length - 1];

        // Décocher tous les articles
        document.querySelectorAll('.article-checkbox').forEach(cb => cb.checked = false);
        document.querySelectorAll('.article-quantity').forEach(q => q.style.display = 'none');

        // Cocher les articles du ravitaillement
        dernierRav.articles.forEach(articleRav => {
            const checkbox = document.querySelector(`.article-checkbox[value="${articleRav.id}"]`);
            if (checkbox) {
                ...
                toggleArticle(articleRav.id, true);
            }
        });

        showNotification(`Ravitaillement ${dernierRav.code} chargé`);
    }
}
...
function toggleArticle(articleId, checked) {
    ...
    if (checked) {
        ...
        const existingIndex = selectedArticles.findIndex(a => a.id === article.id);
        if (existingIndex === -1) {
...
function calculerTotaux() {
    ...
    const totalHT = selectedArticles.reduce((sum, a) => sum + (a.prix * a.quantite), 0);
...
function enregistrerRavitaillement(event) {
    ...
    const articles = [];
    document.querySelectorAll('.rav-article-checkbox').forEach((checkbox, index) => {
        if (checkbox.checked) {
            const articleId = parseInt(checkbox.value);
            const qteInput = document.querySelectorAll('.rav-article-quantite')[index];
    ...
    const newRavitaillement = {
        id: ravitaillements.length > 0 ? Math.max(...ravitaillements.map(r => r.id)) + 1 : 1,
        code: genererCodeRavitaillement(),
        clientId: clientId,
        jour: document.getElementById("ravitaillement-jour").value,
        dateCreation: new Date().toISOString().split('T')[0],
        articles: articles
    };
...
function enregistrerCommande(event) {
    ...
    showNotification(`Proforma ${newProforma.code} créée ! Lien de partage disponible`);
}

function utiliserRavitaillement(ravitaillementId) {
    const ravitaillement = getRavitaillements().find(r => r.id === ravitaillementId);
    if (ravitaillement) {
        openOffcanvas('commande', ravitaillement.clientId);
        setTimeout(() => {
            chargerRavitaillementClient(ravitaillement.clientId);
        }, 500);
    }
}
